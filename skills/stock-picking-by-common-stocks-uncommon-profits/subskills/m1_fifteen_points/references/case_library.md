# M1 15要点评估框架 — 原文引用库
# 知识来源：《Common Stocks and Uncommon Profits and Other Writings》Philip A. Fisher 著，1996年修订版
# 本文件仅包含 Module 1 使用的引用条目，供 module.md 按 REF-ID 引用
# GitHub: https://github.com/simbajigege/book2skills/blob/main/skills/stock-picking-by-common-stocks-uncommon-profits/subskills/m1_fifteen_points/references/case_library.md

---

### REF-M1-001
**类型**：原则
**标题**：两种成长公司："幸运且能干"vs"因能干而幸运"
**触发条件**：用户问一家公司是否能持续增长，或问成长是否只是运气
**适用规则**：真正能持续数十年增长的公司，依靠管理层主动创造下一增长曲线的能力，而非依赖行业顺风

**原书位置**：第3章 Point 1 / 第50-51页

**原文引用**：
> "Those companies which decade by decade have consistently shown spectacular growth might be divided into two groups. For lack of better terms I will call one group those that happen to be both 'fortunate and able' and the other group those that are 'fortunate because they are able.' A high order of management ability is a must for both groups. No company grows for a long period of years just because it is lucky."

**分析逻辑**：
- "幸运且能干"的公司：先有好的行业机遇，管理层有能力把握住
- "因能干而幸运"的公司：管理层的能力主动创造了机会，即使行业不顺也能增长
- Fisher 偏好第二类，因为它的增长更可持续，更少依赖外部环境

**引用指令**：
触发时输出格式：
```
📖 原书引用：[两种成长公司的区分](https://github.com/simbajigege/book2skills/blob/main/skills/stock-picking-by-common-stocks-uncommon-profits/subskills/m1_fifteen_points/references/case_library.md)

> "Those companies which decade by decade have consistently shown spectacular growth might be divided into two groups... No company grows for a long period of years just because it is lucky."
>
> — 《Common Stocks and Uncommon Profits》第50-51页
```

---

### REF-M1-002
**类型**：框架
**标题**：15要点评估框架总纲
**触发条件**：用户问"Fisher 的15要点是什么"，或需要向用户介绍整个评估体系时
**适用规则**：一家公司可以在少数几个要点上不合格仍然是好投资；但若在多个要点上不合格，则不符合 Fisher 定义的值得投资的公司

**原书位置**：第3章 / 第47页

**原文引用**：
> "There are fifteen points with which I believe the investor should concern himself. A company could well be an investment bonanza if it failed fully to qualify on a very few of them. I do not think it could come up to my definition of a worthwhile investment if it failed to qualify on many."

**分析逻辑**：
- 15要点不是清单式逐项打分，而是一个整体判断框架
- 在"很少几项"不合格时仍可能是好投资——关键看是哪几项（诚信不在此列）
- Fisher 强调"整体"而非"平均"——质量最差的那几项往往决定投资价值

**引用指令**：
触发时输出格式：
```
📖 原书引用：[15要点框架总纲](https://github.com/simbajigege/book2skills/blob/main/skills/stock-picking-by-common-stocks-uncommon-profits/subskills/m1_fifteen_points/references/case_library.md)

> "There are fifteen points with which I believe the investor should concern himself. A company could well be an investment bonanza if it failed fully to qualify on a very few of them."
>
> — 《Common Stocks and Uncommon Profits》第47页
```

---

### REF-M1-003
**类型**：原则
**标题**：销售组织是所有业务的基础——Point 4
**触发条件**：用户认为产品好就够了、不重视销售能力，或问如何评估公司销售能力
**适用规则**：即使是最好的产品，若没有优秀的销售组织，也无法实现最大销售潜能；销售能力是任何公司得以生存的基础

**原书位置**：第3章 Point 4 / 第55页

**原文引用**：
> "In this competitive age, the products or services of few companies are so outstanding that they will sell to their maximum potentialities if they are not expertly merchandised. It is the making of a sale that is the most basic single activity of any business. Without sales, survival is impossible."

**分析逻辑**：
- 竞争时代，几乎没有产品好到不需要销售就能卖出最大潜力
- 销售是最基本的商业活动——没有销售就没有生存
- 评估销售组织要通过 Scuttlebutt，从竞争对手和客户处获取

**引用指令**：
触发时输出格式：
```
📖 原书引用：[销售组织是业务基础 Point 4](https://github.com/simbajigege/book2skills/blob/main/skills/stock-picking-by-common-stocks-uncommon-profits/subskills/m1_fifteen_points/references/case_library.md)

> "In this competitive age, the products or services of few companies are so outstanding that they will sell to their maximum potentialities if they are not expertly merchandised. It is the making of a sale that is the most basic single activity of any business."
>
> — 《Common Stocks and Uncommon Profits》第55页
```

---

### REF-M1-004
**类型**：原则
**标题**：最大长期利润永远不来自边际企业——Point 5
**触发条件**：利润率持续低于行业均值，或用户考虑投资一家"低利润但高增速"的公司
**适用规则**：Fisher 明确排除利润率长期偏低的边际企业；唯一例外是有明确迹象显示公司正在发生根本性变化

**原书位置**：第3章 Point 5 / 第53-54页

**原文引用**：
> "I believe that the greatest long-range investment profits are never obtained by investing in marginal companies. The only reason for considering a long-range investment in a company with an abnormally low profit margin is that there might be strong indications that a fundamental change is taking place within the company."

**分析逻辑**：
- 边际利润率 = 弱竞争地位的标志，长期看会侵蚀增长价值
- 唯一例外：有强力证据证明公司正在经历根本性转变（如全新管理层、新核心技术）
- 与 Point 6 配合使用：关键是未来利润率趋势，不是当前绝对数字

**引用指令**：
触发时输出格式：
```
📖 原书引用：[最大利润不来自边际企业 Point 5](https://github.com/simbajigege/book2skills/blob/main/skills/stock-picking-by-common-stocks-uncommon-profits/subskills/m1_fifteen_points/references/case_library.md)

> "I believe that the greatest long-range investment profits are never obtained by investing in marginal companies."
>
> — 《Common Stocks and Uncommon Profits》第53-54页
```

---

### REF-M1-005
**类型**：原则
**标题**：未来利润率比过去利润率更重要——Point 6
**触发条件**：用户以历史利润率判断一家公司的投资价值，或问"现在利润率低但管理层说会改善"
**适用规则**：股票购买的成功不取决于买入时大众已知的信息，而取决于买入后才被市场发现的信息；因此未来利润率改善的可能性比当下绝对数字更重要

**原书位置**：第3章 Point 6 / 第57-58页

**原文引用**：
> "The success of a stock purchase does not depend on what is generally known about a company at the time the purchase is made. Rather it depends upon what gets to be known about it after the stock has been bought. Therefore it is not the profit margins of the past but those of the future that are basically important to the investor."

**分析逻辑**：
- 市场已经定价了过去的利润率；投资者的优势来自对未来改善的早期判断
- 关键问题：管理层是否有具体的利润率改善行动？有无量化目标？
- 与 M3（Scuttlebutt）配合：从供应商/客户处核实管理层的改善行动是否真实

**引用指令**：
触发时输出格式：
```
📖 原书引用：[未来利润率比历史更重要 Point 6](https://github.com/simbajigege/book2skills/blob/main/skills/stock-picking-by-common-stocks-uncommon-profits/subskills/m1_fifteen_points/references/case_library.md)

> "It is not the profit margins of the past but those of the future that are basically important to the investor."
>
> — 《Common Stocks and Uncommon Profits》第57-58页
```

---

### REF-M1-006
**类型**：原则
**标题**：Point 9 授权——管理层干预日常运营是投资危险信号
**触发条件**：CEO 或高管直接处理日常业务细节、管理层强调"我亲自盯每一个项目"
**适用规则**：高管直接干预日常运营，不仅不是认真负责的表现，反而是公司无法扩张的典型障碍

**原书位置**：第3章 Point 9 / 第68-69页

**原文引用**：
> "Those organizations where the top brass personally interfere with and try to handle routine day-to-day operating matters seldom turn out to be the most attractive type of investments. Cutting across the lines of authority which they themselves have set up frequently results in well-meaning executives significantly detracting from the investment caliber of the companies they run."

**分析逻辑**：
- 高管干预日常 = 授权体系失效 = 公司规模扩大时高管成为瓶颈
- 即使是出于好意的干预，也会破坏已建立的权责体系
- 优秀管理层特征：有清晰的授权框架，专注战略而非日常运营

**引用指令**：
触发时输出格式：
```
📖 原书引用：[管理层授权 Point 9](https://github.com/simbajigege/book2skills/blob/main/skills/stock-picking-by-common-stocks-uncommon-profits/subskills/m1_fifteen_points/references/case_library.md)

> "Those organizations where the top brass personally interfere with and try to handle routine day-to-day operating matters seldom turn out to be the most attractive type of investments."
>
> — 《Common Stocks and Uncommon Profits》第68-69页
```

---

### REF-M1-007
**类型**：原则
**标题**：Point 15 诚信——一票否决，无条件
**触发条件**：出现任何管理层诚信疑虑，或用户问"诚信问题但其他方面都很好，能投资吗"
**适用规则**：诚信是唯一不能被其他要点的高分所弥补的要点；一旦诚信存疑，无论其他评分多高，立即排除

**原书位置**：第3章 Point 15 / 第77-78页

**原文引用**：
> "There is only one real protection against abuses like these. This is to confine investments to companies the managements of which have a highly developed sense of trusteeship and moral responsibility to their stockholders. Any investment may still be considered interesting if it falls down in regard to almost any other one of the fifteen points which have now been covered, but rates an unusually high score in regard to all the rest. Regardless of how high the rating may be in all other matters, however, if there is a serious question of the lack of a strong management sense of trusteeship for stockholders, the investor should never seriously consider participating in such an enterprise."

**分析逻辑**：
- Fisher 用"唯一真正的保护"（only one real protection）来描述诚信要求——不是建议，是硬性标准
- 其他14个要点都可以有例外，唯独 Point 15 不行
- "serious question"（严重疑问）就足够触发否决，不需要已有明确证据

**引用指令**：
触发时输出格式：
```
📖 原书引用：[诚信一票否决 Point 15](https://github.com/simbajigege/book2skills/blob/main/skills/stock-picking-by-common-stocks-uncommon-profits/subskills/m1_fifteen_points/references/case_library.md)

> "Regardless of how high the rating may be in all other matters, however, if there is a serious question of the lack of a strong management sense of trusteeship for stockholders, the investor should never seriously consider participating in such an enterprise."
>
> — 《Common Stocks and Uncommon Profits》第77-78页
```
