# M5 卖出/持有决策 — 原文引用库
# 知识来源：《Common Stocks and Uncommon Profits and Other Writings》Philip A. Fisher 著，1996年修订版
# 本文件仅包含 Module 5 使用的引用条目，供 module.md 按 REF-ID 引用
# GitHub: https://github.com/simbajigege/book2skills/blob/main/skills/common-stocks-philip-fisher/subskills/m5_when_to_sell/references/case_library.md

---

### REF-M5-001
**类型**：框架
**标题**：三个理由，且仅有三个理由——Fisher 卖出决策框架
**触发条件**：用户考虑卖出任何持仓时，需要检验卖出理由是否符合 Fisher 标准
**适用规则**：只有三种情况构成卖出的充分理由；其他所有理由（市场、估值、涨幅）均不是有效理由

**原书位置**：第6章 / 第105-106页

**原文引用**：
> "I believe there are three reasons, and three reasons only, for the sale of any common stock which has been originally selected according to the investment principles already discussed. The first of these reasons should be obvious to anyone. This is when a mistake has been made in the original purchase and it becomes increasingly clear that the factual background of the particular company is, by a significant margin, less favorable than originally believed."

**分析逻辑**：
- Fisher 使用"三个理由，且仅有三个理由"（three reasons, and three reasons only）这样的强调句式
- 前置条件：这套标准仅适用于"按照已讨论的投资原则选择的股票"——即通过了 M1 质量评估的公司
- 任何其他理由（市场预期、估值、涨幅）都不在这三个理由之列

**引用指令**：
触发时输出格式：
```
📖 原书引用：[Fisher 三个有效卖出理由](https://github.com/simbajigege/book2skills/blob/main/skills/common-stocks-philip-fisher/subskills/m5_when_to_sell/references/case_library.md)

> "I believe there are three reasons, and three reasons only, for the sale of any common stock which has been originally selected according to the investment principles already discussed."
>
> — 《Common Stocks and Uncommon Profits》第6章，第105-106页
```

---

### REF-M5-002
**类型**：警示
**标题**：因熊市恐惧卖出——错误比不卖更大
**触发条件**：用户担忧市场将要下跌，想先卖出等市场见底后再买回
**适用规则**：卖出后想在更低价格买回的计划几乎不可能实现；因熊市担忧卖出优质成长股，长期看是更大的错误

**原书位置**：第6章 / 第108-109页

**原文引用**：
> "I have seen many investors dispose of a holding that was to show stupendous gain in the years ahead because of this fear of a coming bear market. Frequently the bear market never came and the stock went right on up. When a bear market has come, I have not seen one time in ten when the investor actually got back into the same shares before they had gone up above his selling price."

**分析逻辑**：
- 两种错误情形：(1) 预期中的熊市根本没来 (2) 熊市来了但投资者没能在更低价格买回
- "10次中连一次都没有见到"（not seen one time in ten）是 Fisher 的强力经验性断言
- 结合资本利得税：卖出意味着交税，再买回需要股价低于卖出价 × (1 - 税率) 才算盈亏平衡

**引用指令**：
触发时输出格式：
```
📖 原书引用：[因熊市恐惧卖出的后果](https://github.com/simbajigege/book2skills/blob/main/skills/common-stocks-philip-fisher/subskills/m5_when_to_sell/references/case_library.md)

> "When a bear market has come, I have not seen one time in ten when the investor actually got back into the same shares before they had gone up above his selling price."
>
> — 《Common Stocks and Uncommon Profits》第6章，第108-109页
```

---

### REF-M5-003
**类型**：原则
**标题**：股价已大幅上涨不是卖出理由——劳务合同类比
**触发条件**：用户因股价已大幅上涨（"已经赚了很多了"）而考虑卖出
**适用规则**：优秀公司股价的上涨反映了业绩的真实改善；就像一个优秀员工晋升了不代表应该解除合同，股价上涨不代表应该卖出

**原书位置**：第6章 / 第109-110页

**原文引用**：
> "Outstanding companies, the only type which I believe the investor should buy, just don't function this way. If you know your common stocks equally well, many of the arguments commonly heard for selling the good one sound equally silly."

**分析逻辑**：
- Fisher 的类比：你在大学时与同学签订了一份劳务合同；当他被晋升为副总裁、薪资大幅上涨后，你不会因为"他的薪资已经涨了600%"就卖掉这份合同
- 股票的逻辑一样：优秀公司的股价上涨，是其价值的体现，不是卖出的理由
- 关键问题始终是：**公司是否仍然符合 Fisher 的15要点**，而非股价已涨了多少

**引用指令**：
触发时输出格式：
```
📖 原书引用：[股价上涨不是卖出理由](https://github.com/simbajigege/book2skills/blob/main/skills/common-stocks-philip-fisher/subskills/m5_when_to_sell/references/case_library.md)

> "Outstanding companies, the only type which I believe the investor should buy, just don't function this way. Many of the arguments commonly heard for selling the good one sound equally silly."
>
> — 《Common Stocks and Uncommon Profits》第6章，第109-110页
```

---

### REF-M5-004
**类型**：警示
**标题**：等回本再卖——投资中最昂贵的心理偏差
**触发条件**：用户持有一个亏损的错误仓位，不愿认亏，想等到至少回本再卖
**适用规则**："等回本再卖"是自我欺骗导致的最昂贵的单一损失来源；正确做法是一旦确认失误就立即卖出

**原书位置**：第6章 / 第106-107页

**原文引用**：
> "More money has probably been lost by investors holding a stock they really did not want until they could 'at least come out even' than from any other single reason. If to these actual losses are added the profits that might have been made through the proper reinvestment of these funds if such reinvestment had been made when the mistake was first realized, the cost of self-indulgence becomes truly tremendous."

**分析逻辑**：
- 两层损失：(1) 等待回本期间的直接亏损继续扩大；(2) 这些资金本可以投入更好的机会所产生的机会成本
- "self-indulgence"（自我放纵）——Fisher 用这个词描述"等回本"的心理，含义深刻：这不是理性决策，是情感上对自我的纵容
- 真正的问题不是"我亏了多少"，而是"这些资金现在的最佳去处是哪里"

**引用指令**：
触发时输出格式：
```
📖 原书引用：[等回本是最昂贵的心理偏差](https://github.com/simbajigege/book2skills/blob/main/skills/common-stocks-philip-fisher/subskills/m5_when_to_sell/references/case_library.md)

> "More money has probably been lost by investors holding a stock they really did not want until they could 'at least come out even' than from any other single reason."
>
> — 《Common Stocks and Uncommon Profits》第6章，第106-107页
```

---

### REF-M5-005
**类型**：框架
**标题**："几乎永不卖出"——Fisher 持有哲学的核心总结
**触发条件**：用户问 Fisher 的总体持有哲学是什么，或想用一句话概括什么时候卖股票
**适用规则**：如果买入时做了正确的判断，持有决策的答案几乎永远是不卖；这是 Fisher 方法的终极逻辑

**原书位置**：第6章 / 第113页

**原文引用**：
> "Perhaps the thoughts behind this chapter might be put into a single sentence: If the job has been correctly done when a common stock is purchased, the time to sell it is—almost never."

**分析逻辑**：
- "almost never"（几乎永不）是 Fisher 整个投资哲学的压缩
- 前提条件："job has been correctly done"——买入时的调研工作做到位
- 逻辑链：正确的公司（通过15要点）→ 有了正确的持有理由 → 只要理由不变，持有决策不变
- 这也意味着：频繁的卖出决策是对自己买入能力的怀疑，或者买入时本就没有做足工作

**引用指令**：
触发时输出格式：
```
📖 原书引用：[Fisher 持有哲学总结](https://github.com/simbajigege/book2skills/blob/main/skills/common-stocks-philip-fisher/subskills/m5_when_to_sell/references/case_library.md)

> "If the job has been correctly done when a common stock is purchased, the time to sell it is—almost never."
>
> — 《Common Stocks and Uncommon Profits》第6章，第113页
```

---

### REF-M5-006
**类型**：框架
**标题**：持续持有测试——下一个商业周期峰值盈利验证法
**触发条件**：用户不确定是否应该继续持有，需要一个具体的测试方法来做决定
**适用规则**：如果预期在下一个商业周期峰值时，每股盈利的增幅不低于当前水平相比上一个峰值的增幅，则继续持有；否则考虑卖出

**原书位置**：第6章 / 第111页

**原文引用**：
> "There is a good test as to whether companies no longer adequately qualify in regard to this matter of expected further growth. This is for the investor to ask himself whether at the next peak of a business cycle, regardless of what may happen in the meantime, the comparative per-share earnings... will probably show at least as great an increase from present levels as the present levels show from the last known peak of general business activity. If the answer is in the affirmative, the stock probably should be held. If in the negative, it should probably be sold."

**分析逻辑**：
- 测试设计巧妙：排除短期波动，聚焦长期盈利成长轨迹
- "regardless of what may happen in the meantime"——中间的市场波动无关紧要，只看峰值到峰值的增长
- 如果这个测试的答案是"不确定"，那么需要做 Scuttlebutt 核实，而不是立即卖出

**引用指令**：
触发时输出格式：
```
📖 原书引用：[持续持有测试方法](https://github.com/simbajigege/book2skills/blob/main/skills/common-stocks-philip-fisher/subskills/m5_when_to_sell/references/case_library.md)

> "This is for the investor to ask himself whether at the next peak of a business cycle... the comparative per-share earnings will probably show at least as great an increase from present levels as the present levels show from the last known peak. If the answer is in the affirmative, the stock probably should be held."
>
> — 《Common Stocks and Uncommon Profits》第6章，第111页
```
