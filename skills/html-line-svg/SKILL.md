...
3. 由其他 skill 调用时，回到调用它的上层工作流：遵守上层 skill 规定的事实来源、源文件、素材登记、追溯记录、生成物和验证范围，不固定绑定 `tailor-html-to-slide`、`ppt-like-html` 或任何单一载体。独立调用时，遵守目标项目与目标文件所在目录的规则。
...
6. 可访问性：确认 `<title>`、`<desc>` 与 `aria-labelledby` 对应，正文即使不看图也能理解核心判断。
